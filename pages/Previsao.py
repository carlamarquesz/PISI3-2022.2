...
def main():
    ...
    if enviar:   
        ...
        resultado = executar_classificador(classificador_svm, X_train, [solicitante], y_train)
        print(resultado)
        if resultado == 0: 
        ...
        metricas = {
            ...
            'Valor': [acuracia_svm, precisao_svm, recall_svm]
        }
        with st.expander("Resultados da Validação SVM"):
            st.subheader("Métricas:")
            st.table(pd.DataFrame(metricas))

            st.subheader("Matriz de Confusão:")
            st.table(matrix_confusao_svm) 
